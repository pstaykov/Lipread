...
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))  # dataset.py flattened alongside this file (handoff copy)
...
def _ensure_ffmpeg():
    ...
    try:
        ...
        # whisper calls ffmpeg by name; monkey-patch it to the full path (no symlinks on Windows)
...
if __name__ == '__main__':
    ...
    CHECKPOINT_EVERY = 5
    # Regularization run: warm-starts from the earlier multimodal checkpoint, which
    # peaked early and then memorised. A fresh SAVE_DIR avoids resuming its overfit state.
    ...
    NUM_FRAMES = 25
    # Shorter schedule than the original 40-epoch run: a longer anneal let train
    # accuracy keep climbing while val fell every epoch (capacity-driven overfit,
    # not LR-driven), so this pairs a shorter run with early stopping instead.
    NUM_EPOCHS = 15
    WARMUP_EPOCHS = 3
    EARLY_STOP_PATIENCE = 4
    # Freeze the visual backbone briefly so the randomly-initialised cross-attention
    # trains against a stable visual representation before backbone gradients start.
    ...
    # Peak LRs lowered from the original schedule, whose higher warmup target was
    # unlearning the transferred visual features (train and val degraded together).
    param_groups = [
    ...
    if start_epoch == 0 or not os.path.exists(metrics_path):
        ...
        # Drop any metrics row past the resume point so the CSV matches the loaded weights.
    ...
    for epoch in epoch_bar:
        ...
        # Toggled every epoch (not once) so a resume lands in the correct state.
