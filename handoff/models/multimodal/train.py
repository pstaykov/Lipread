"""Warm-start the multimodal fusion model on the full 500-class GLips set from the
earlier shipped checkpoint (models/checkpoints_reg/best_model.pth), instead of
retraining audio+visual+cross-attn from scratch.
...
class list dynamically instead of hardcoding an exclusion.

Every tensor in the shipped checkpoint except the final `classifier` is class-count
agnostic — visual trunk, MS-TCN, Transformer, and the trained cross-attention/audio_proj
all carry over by name+shape via load_visual_weights (despite the name, it loads ANY
matching-shaped tensor, so pointing it at the full fused checkpoint instead of a
visual-only one warm-starts the whole model). Only the classifier starts fresh, so this
needs far fewer epochs than the original 15-epoch run: the backbone is frozen for 1
epoch just to let the new head settle.

HANDOFF COPY: this is the exact script that trained the shipped best_model.pth in
this folder (72.0% test top-1, fully fine-tuned, 500-class GLips). `model.py`
alongside it here is the original repo's `train.py` (it holds GLipsNet + the
dataset + the Whisper extractor, hence the import below). PRETRAIN_CKPT, ROOT_DIR,
AUDIO_ROOT and AUDIO_CACHE_DIR point at the full-repo layout (the earlier warm-start
checkpoint, ~250k-clip GLips corpus, 16GB audio waveform cache) and are not shipped
in this handoff. Point them at your own copies to rerun.
...
PRETRAIN_CKPT = './checkpoints_reg/best_model.pth'  # not shipped, see note above
SAVE_DIR = './checkpoints_500'
ROOT_DIR = './GLips_mouth/lipread_files'  # not shipped, see note above
AUDIO_ROOT = './GLips/lipread_files'  # not shipped, see note above
AUDIO_CACHE_DIR = './audio_cache'  # not shipped, see note above
NUM_FRAMES = 25
NUM_EPOCHS = 8
WARMUP_EPOCHS = 1
...
def main():
    ...
    if not os.path.exists(PRETRAIN_CKPT):
        raise SystemExit(f"Fused checkpoint not found at {PRETRAIN_CKPT}.")
    ...
    # overwritten by the earlier weights. load_visual_weights loads any name+shape
    # match, so a mismatched classifier row count is skipped automatically.
